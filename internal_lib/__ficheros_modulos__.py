...
def CambiarEstadoIDxlsx(dirExcel, ID, colnames, estado, ncol_ID=0, ncol_ESTADO=1, except_kill=False):
  ...
  try:
    ...
    if except_kill: raise ExcepcionArchivo(f"Error al cambiar el estado del ID, la cantidad de columnas no coincide para {os.path.basename(dirExcel)}")
  ...
  while True:
    try:
        ...
        if except_kill: raise ExcepcionArchivo(f"Error de permisos, se abrio el archivo {os.path.basename(dirExcel)} con otra aplicación, favor cerrar y presionar enter para continuar")
        else: 
          notify("Error de permisos", "Se abrio el archivo con otra aplicación, favor cerrar y presionar enter para continuar")
          input(f"Error de permisos, se abrio el archivo {os.path.basename(dirExcel)} con otra aplicación, favor cerrar y presionar enter para continuar")
...
def ReestablecerXLSX(dirExcel, colnames, filtro=False):
  while True:
    try:
        ...
        notify("Error de permisos", "Se abrio el archivo con otra aplicación, favor cerrar y presionar enter para continuar")
        input(f"Cierre el archivo excel {os.path.basename(dirExcel)} para continuar\n")
...
def AbrirXLSX(dirExcel, colnames, except_kill=True, except_create=True, except_alert=True):
    try:
        ...
        if colnames != regExcel.columns.to_list() and except_alert:
            if except_kill: raise ExcepcionArchivo(f"La cantidad de columnas no coincide para {os.path.basename(dirExcel)}, los datos pueden quedar corridos\nSe esparaba: {colnames}\nSe lee: {regExcel.columns.to_list()}\n")
            else: 
      ...
      if not except_create: raise ExcepcionArchivo(f"No se encontro el archivo {os.path.basename(dirExcel)}")
      else:
        if except_alert: print(f"No se encontro el archivo {os.path.basename(dirExcel)}, se creara uno nuevo...\n")

    except BadZipFile as e:
        print(f"El archivo {os.path.basename(dirExcel)} parece estar corrupto debido a un cierre inesperado al guardar\nVolviendo a crear el archivo...\n")
        os.remove(dirExcel)
...
def ListaAColumnaXLSX(dirExcel, valores_fila, colname, colnames, except_kill=False, except_create=True, funcion_print=print):
  ...
  try:    
    ...
    if except_kill: 
      raise ExcepcionArchivo(f"La cantidad de columnas no coincide para {os.path.basename(dirExcel)}")
    else:
      funcion_print(f"Error al guardar el archivo {os.path.basename(dirExcel)}, la cantidad e columnas no coincide {e}")
...
def FilaAgregarXLSX(dirExcel, valores_fila, colnames, except_kill=False, except_create=True):
  ...
  try:    
    ...
    if except_kill: 
      raise ExcepcionArchivo(f"La cantidad de columnas no coincide en {os.path.basename(dirExcel)}")
    else:
      print(f"Error al guardar el archivo {os.path.basename(dirExcel)}, la cantidad e columnas no coincide {e}")
...
def AgregarMuestraXLSX(dirExcel, ID_Muestra, colnames, ID_controles=["-"], except_kill=False, except_create=False):
    ...
    while True:
        try:
            ...
            if except_kill: raise ExcepcionArchivo(f"La cantidad de columnas no coincide en {os.path.basename(dirExcel)}")
            else:
              notify("Error de permisos", "Se abrio el archivo con otra aplicación, favor cerrar y presionar enter para continuar")  
              input(f"Error de permisos, se abrio el archivo {os.path.basename(dirExcel)} con otra aplicación, favor cerrar y presionar enter para continuar")
...
def ListaMuestraXLSX(dirExcel, colname, colnames, except_kill=False, except_create=True, except_alert=False):
    ...
    try:
      ...
      if except_kill: raise ExcepcionArchivo(f"Error al obtener lista, la columna {colname} del excel {os.path.basename(dirExcel)} no existe")
      else: 
        print(f"Error al obtener lista, la columna {colname} del excel {os.path.basename(dirExcel)} no existe")
        return []
